...
import com.failframe.collector.repository.RootCauseAnalysisRepository;
...
class FailureIngestionApiIntegrationTest {
    ...
    private static final String LOCAL_API_KEY = "failframe-local-dev-key";
    private static final String ADMIN_USER = "failframe";
    private static final String ADMIN_PASSWORD = "failframe";

    @Container
    static final PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>("postgres:16-alpine")
            .withDatabaseName("failframe_test")
            .withUsername("failframe")
            .withPassword("failframe");
    ...
    static void postgresProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("failframe.security.username", () -> ADMIN_USER);
        registry.add("failframe.security.password", () -> ADMIN_PASSWORD);
    }
    ...
    void invalidApiKeyReturns403() throws Exception {
        mockMvc.perform(post("/api/v1/events/failures")
                        .header("X-FailFrame-Api-Key", "not-a-valid-key")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(defaultPayload("local")))
    ...
    void sensitiveHeadersAreMasked() throws Exception {
        ...
        assertThat(saved.getRequestHeadersJson().get("X-FailFrame-Api-Key").asText()).isEqualTo("[masked]");
    ...
    private ResultActions performIngest(String payload, String traceparent) throws Exception {
        var requestBuilder = post("/api/v1/events/failures")
                .header("X-FailFrame-Api-Key", LOCAL_API_KEY)
                .contentType(MediaType.APPLICATION_JSON)
                .content(payload);
        if (traceparent != null) {
