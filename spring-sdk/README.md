# FailFrame Spring Boot SDK

This module is a Spring Boot 3 starter-style SDK. Add it to a servlet-based Spring Boot app and it automatically captures failed HTTP requests and sends them to the FailFrame collector.
...
- Masks sensitive headers before sending: `Authorization`, `Cookie`, `Set-Cookie`, `X-Api-Key`, `X-FailFrame-Api-Key`.
...
FailFrame does not require OpenTelemetry, but it can preserve trace context from services that already use it.
...
If an incoming request includes a `traceparent` header, FailFrame stores that trace ID and span ID on the failure event. If no header exists and `io.opentelemetry.api.trace.Span.current()` is available to the application, FailFrame uses the current span context. If neither source is available, FailFrame generates trace/span IDs so the event remains searchable.
